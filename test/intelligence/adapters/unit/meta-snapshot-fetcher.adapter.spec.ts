...
function makeCampaignModel(campaignDoc: unknown, productResolverDocs: unknown[] = []) {
  return {
...
describe('MetaSnapshotFetcherAdapter', () => {
  it('throws when campaign not found', async () => {
    ...
    const adapter = new MetaSnapshotFetcherAdapter(
      companies as any, campaignModel as any, briefModel as any,
    );
    await expect(
      adapter.fetch({ tenantId: 'astro', campaignId: 'missing', metaCampaignId: 'mc1' }),
  ...
  it('throws when tenant not found', async () => {
    ...
    await expect(
      adapter.fetch({ tenantId: 'astro', campaignId: 'c', metaCampaignId: 'mc1' }),
  ...
  it('emits a well-formed RawMetaBundle with campaign + adSet + ad tiers, no Meta calls', async () => {
    ...
    const companies = makeCompanies({
      tenantId: 'astro',
      products: [{ name: 'Kundli', active: true, conversionValue: 999, refundRatePercent: 0 }],
    });
    const adapter = new MetaSnapshotFetcherAdapter(
      companies as any, campaignModel as any, briefModel as any,
    );
    ...
    expect(bundle.ads['ad1'].insights?.video_p25_watched_actions).toEqual([{ value: 3000 }]);
  ...
  it('reverses the refund haircut so SnapshotBuilder — which applies its own — lands on the right net figure', async () => {
    ...
    const companies = makeCompanies({
      tenantId: 'astro',
      products: [{ name: 'Kundli', active: true, conversionValue: 999, refundRatePercent: 5 }],
    });
    const adapter = new MetaSnapshotFetcherAdapter(
      companies as any, campaignModel as any, briefModel as any,
    );
    ...
    const grossValue = Number(bundle.campaign.insights?.action_values?.[0].value);
  ...
  it('resolves the product via the campaign brief when one is set, not just the first active product', async () => {
    ...
    const briefModel = makeBriefModel([{ briefId: 'brief-1', product: 'Nadi' }]);
    const companies = makeCompanies({
      tenantId: 'astro',
      products: [
        { name: 'Kundli', active: true, conversionValue: 999, refundRatePercent: 0 },
        { name: 'Nadi', active: true, conversionValue: 1799, refundRatePercent: 10 },
      ],
    });
    const adapter = new MetaSnapshotFetcherAdapter(
      companies as any, campaignModel as any, briefModel as any,
